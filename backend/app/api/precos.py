...
def listar_precos(clienteId: int, db: Session = Depends(get_db)):
    rows = db.execute(
...
def lookup(clienteId: int, produtoId: int, db: Session = Depends(get_db)):
...
def salvar_preco(payload: PrecoClienteUpsert, db: Session = Depends(get_db)):
